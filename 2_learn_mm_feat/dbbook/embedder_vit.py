...
for path in tqdm(paths, total=len(paths)):
  ...
  try:
    ...
    name = int(path.split('.')[0])

    image = Image.open('_images/'+path).convert('RGB')
...
pickle.dump(embs_cls, open('images/vit_cls.pkl', 'wb'))
pickle.dump(embs_avg, open('images/vit_avg.pkl', 'wb'))