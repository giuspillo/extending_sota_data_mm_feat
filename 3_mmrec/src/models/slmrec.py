...
class SLMRec(GeneralRecommender):
    ...
    def compute(self):
        ...
        if self.config["dataset"] == "kwai" or self.t_feat is None:
    ...
    def feature_dropout(self, users_idx, items_idx):
        ...
        if self.config["data.input.dataset"] != "kwai" and self.t_feat is not None:
        ...
        if self.config["data.input.dataset"] == "kwai" or self.t_feat is None:
    ...
    def feature_masking(self, users_idx, items_idx, dropout=False):
        ...
        rand_range = 4 if self.config["data.input.dataset"] != "kwai" and  self.t_feat is not None else 2
